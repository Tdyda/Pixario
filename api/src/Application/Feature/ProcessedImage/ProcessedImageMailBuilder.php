...
final readonly class ProcessedImageMailBuilder
{
    /**
     * @param ResultResponse[] $results
     */
    public function buildSubject(string $galleryName, array $results): string
    {
        $failedCount = $this->countFailed($results);

        if ($failedCount === 0) {
            return 'Pixario · Przetwarzanie zdjęć zakończone: ' . $galleryName;
        }

        return 'Pixario · Część zdjęć wymaga uwagi: ' . $galleryName;
    }

    /**
     * @param ResultResponse[] $results
     */
    private function countFailed(array $results): int
    {
        return count(
            array_filter(
                $results,
                fn(ResultResponse $result) => $result->status !== 'Done'
            )
        );
    }
    ...
    public function buildHtml(string $galleryName, array $results): string
    {
        ...
        $successCount = count($results) - $failedCount;

        $statusTitle = $failedCount === 0
            ? 'Przetwarzanie zakończone pomyślnie'
            : 'Przetwarzanie częściowo zakończone';

        $statusDescription = $failedCount === 0
            ? 'Wszystkie zdjęcia zostały poprawnie przetworzone przez Pixario AI.'
            : 'Niektórych zdjęć nie udało się poprawnie przetworzyć.';

        $statusBadge = $failedCount === 0
            ? '<span style="display:inline-block;padding:8px 14px;border-radius:999px;background:#e8fff1;color:#0d8a4b;font-size:12px;font-weight:700;">✔ Wszystkie zdjęcia gotowe</span>'
            : '<span style="display:inline-block;padding:8px 14px;border-radius:999px;background:#fff1f1;color:#d33b3b;font-size:12px;font-weight:700;">⚠ Wymagana uwaga</span>';
                    ...
                    <tr>
                        <td align="center">
                            <table width="100%%" cellpadding="0" cellspacing="0" style="max-width:720px;background:#ffffff;border-radius:28px;overflow:hidden;box-shadow:0 24px 70px rgba(30,19,72,0.12);">
                                ...
                                <tr>
                                    <td style="padding:48px;background:
                                        radial-gradient(circle at 78%% 28%%, rgba(255,255,255,0.24), transparent 10%%),
                                        linear-gradient(135deg,#160b4b 0%%,#42158b 48%%,#bd82ef 100%%);
                                        color:#ffffff;">
                                                ...
                                                <tr>
                                                    <td width="50%%" style="padding-right:8px;">
                                                        <div style="padding:18px;border-radius:16px;background:#ffffff;border:1px solid #eee8f7;text-align:center;">
                                                            <div style="font-size:32px;font-weight:800;color:#6f35e8;">
                                                                %d
                                                            </div>
                                                    ...
                                                    <td width="50%%" style="padding-left:8px;">
                                                        <div style="padding:18px;border-radius:16px;background:#ffffff;border:1px solid #eee8f7;text-align:center;">
                                                            <div style="font-size:32px;font-weight:800;color:%s;">
                                                                %d
                                                            </div>
                                                            <div style="margin-top:6px;font-size:13px;font-weight:700;color:#8a839b;">
                                                                Błędów
